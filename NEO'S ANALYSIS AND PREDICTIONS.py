...
for j in tl:
  ...
  d1.append(l1)
  G.append(G1)
df_the_largest=pd.DataFrame({"Asteroid":tl,"Estimated_diameter":l,"CA Date in near future":d1})
...
x_train,x_test,y_train,y_test = sklearn.model_selection.train_test_split(X,Y,test_size=0.43,random_state=2)
...
plt.show()
print("NEO:(2019 BE5)")
print("Analysed:likely to enter earth's atmosphere in 2881")
p = y_predictnew = lm.predict([[2881, a, 25.1]])
print(p)
